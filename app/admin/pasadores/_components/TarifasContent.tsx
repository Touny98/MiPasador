...
export function TarifasContent() {
  ...
  function handleEdit(tarifa: any) {
    setEditId(tarifa.id);
    setForm({ ruta: tarifa.ruta, peso_min: tarifa.peso_min, peso_max: tarifa.peso_max, precio_ars: tarifa.precio_ars, activa: tarifa.activa });
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-bold mb-2">{editId ? 'Editar Tarifa' : 'Nueva Tarifa'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">Ruta:</label>
            <select name="ruta" value={form.ruta} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">Peso mínimo (kg):</label>
              <input type="number" name="peso_min" value={form.peso_min ?? ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded" min="0" step="0.1" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Peso máximo (kg):</label>
              <input type="number" name="peso_max" value={form.peso_max ?? ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded" min="0" step="0.1" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Precio (ARS):</label>
            <input type="number" name="precio_ars" value={form.precio_ars ?? ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded" min="0" step="0.01" />
          </div>
          <div className="flex items-center">
            <input type="checkbox" name="activa" checked={form.activa} onChange={handleChange} className="h-4 w-4 text-blue-600" />
            <label className="ml-2 text-sm font-medium">Activa</label>
          </div>
          <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            {editId ? 'Guardar cambios' : 'Agregar tarifa'}
          </button>
          {editId && (
            <button type="button" onClick={() => { setEditId(null); setForm({ ruta: '', peso_min: null, peso_max: null, precio_ars: null, activa: true }); }} className="ml-2 px-4 py-2 bg-gray-300 text-gray-800 rounded">
              Cancelar
            </button>
          )}
        </form>
          ...
          {tarifas.map((t) => (
            <tr key={t.id} className="border-t">
              ...
              <td className="p-4 text-center">
                <span className={`px-2 py-1 rounded text-xs ${t.activa ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {t.activa ? 'Sí' : 'No'}
                </span>
              </td>
              <td className="p-4 text-center space-x-2">
                <button onClick={() => handleEdit(t)} className="px-3 py-1 rounded text-sm bg-blue-500 text-white hover:bg-blue-600">Editar</button>
                <button onClick={() => handleDelete(t.id)} className="px-3 py-1 rounded text-sm bg-red-500 text-white hover:bg-red-600">Eliminar</button>
              </td>
            </tr>
          ))}
          {tarifas.length === 0 && (
            <tr>
              <td colSpan={6} className="p-4 text-center text-gray-500">No hay tarifas</td>
            </tr>
          )}
        </tbody>
      </table>
