...
export function ViajesContent() {
  ...
  async function fetchViajes() {
    ...
    if (filters.fechaDesde) {
      query = query.gte('creado_at', filters.fechaDesde);
    }

    if (filters.fechaHasta) {
      query = query.lte('creado_at', filters.fechaHasta);
    }
    ...
    if (error) {
      ...
      // Process data to flatten the relations
          ...
          {viajes.map((v) => (
            <tr key={v.id} className="border-t">
              ...
              <td className="p-4 text-center">
                {/* Comisión: 10% del precio */}
                ${(v.precio * 0.1).toFixed(2)}
              </td>
              <td className="p-4 text-center">
